...
* cette commande :
```
$ FILENAME="iotop-totaldisk-20200719T224932.log"; grep -E "Total DISK" $FILENAME | sed -E 's/\||(K\/s)|(Total DISK READ :)|(Total DISK WRITE :)//g' | awk -F" " -v logdate="Jul 9, 2020" 'BEGIN { OFS="," }; \
{ \                  
split($1,time,":"); \                                                
cmd ="date \"+%s\" -d \""logdate" "time[1]":"time[2]":"time[3]"\""; \
cmd | getline var; \
...
* commande complète, avec sauvegarde dans un fichier csv
```
$ FILENAME="iotop-totaldisk-20200719T224932.log"; echo "unixepochtime,totaldiskread,totaldiskwrwite" > ${FILENAME}.csv; grep -E "Total DISK" $FILENAME | sed -E 's/\||(K\/s)|(Total DISK READ :)|(Total DISK WRITE :)//g' | awk -F" " -v logdate="Jul 9, 2020" 'BEGIN { OFS="," }; \
{ \                  
split($1,time,":"); \                                                
cmd ="date \"+%s\" -d \""logdate" "time[1]":"time[2]":"time[3]"\""; \
cmd | getline var; \
