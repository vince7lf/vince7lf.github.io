...
```
echo "ramused,ramtotal,swapused,swaptotal,cached,cpu1per,cpu1freq,cpu2per,cpu2freq,cpu3per,cpu3freq,cpu4per,cpu4freq,emcfreq,gr3dfreq,plltemp,cputemp,pmictemp,gputemp,aotemp,thermal,pom5vincurr,pom5vinavg,pom5vgpucurr,pom5vgpuavg,pom5vcpucurr,pom5vcpuavg" | cat - tegrastats-20200719T220319.log | sed -E 's/[()%\[]|MB|]|kB/ /g' | sed -E 's/[,@]|C / /g' | sed -E 's/ /\t/g' | sed -E 's/\t+/\t/g' | awk -F$"\t" 'BEGIN { OFS="," }; \
{ \
split($3,RAM,"/"); \
split($7,SWAP,"/"); \
split($36,POM_5V_IN,"/"); \
...
END {}; \' | sed -E 's/,/\t/g'
ramused	swapused		cpu1freq		cpu2freq	cpu3freq	cpu4per	cpu4freq	emcfreq	gr3dfreq	plltemp	cputemp	pmictemp	aotemp	pom5vincurr	pom5vgpucurr	pom5vcpucurr										
